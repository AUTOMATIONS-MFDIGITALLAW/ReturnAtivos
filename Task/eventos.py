...
def eventos():
    ...
    texto = 'Prezados, por gentileza, encaminhar informações adicionais, caso necessário: Fatura/ Vision/ dados do lojista/ Recupera'
    pyperclip.copy(texto)
    pyautogui.hotkey('ctrl', 'v')
    time.sleep(3)
    ...
    click_and_fill('eventos','caminho eventos encontrado', 'Caminho eventos não encontrado')
    pya.write('JONE REIS DE PAIVA FREITAS')
    searchimage('jone', 'jone ancontrada', 'jone não encontrada')
    time.sleep(4)
    searchimage('setadireita2', 'setadireita ancontrada', 'seta não encontrada')
...
def eventosPrazo():
    ...
    time.sleep(3)
    searchimage('encaminhamento', 'encaminhamento encontrada', 'encaminhamento não encontrado')
    click_and_fill('data da citacao','caminho vazio encontrado', 'Caminho vazio não encontrado')
